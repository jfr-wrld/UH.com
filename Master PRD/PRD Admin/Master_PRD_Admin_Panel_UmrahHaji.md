...
flowchart TD
    ...
    COMMS --> ANN["Announcement Management"]
    ...
    ANN --> ADASH["Announcement Dashboard"]
    ANN --> ALIST2["All Announcements"]
    ANN --> ACREATE["Create Announcement"]
    ANN --> ATPL["Templates (Phase 2)"]
    ANN --> ASET2["Announcement Settings"]
...
5. Announcement Management uses `Published` as a high-level stakeholder term, while the module PRD uses `Sent` for the operational delivery status. Published announcements are immutable; unpublish behavior means archive, expire, or hide from active views while retaining audit history.
...
| Announcement Management | Manage platform announcements, targeted audiences, send/schedule workflow, delivery tracking, acknowledgement, archive/expiry, and announcement settings |
