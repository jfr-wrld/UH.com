...
import { DollarSign, TrendingUp, TrendingDown, AlertCircle, RefreshCw, Download, Plus, FileText, Settings, CheckCircle, Clock } from 'lucide-react';

export const AdminFinanceDashboard: React.FC<{ navigate: (path: string, state?: any) => void }> = ({ navigate }) => {
      ...
      <PageHeader 
        ...
        actions={
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            ...
            <Button variant="secondary" leftIcon={<FileText size={16} />} onClick={() => navigate('admin-finance-payouts')}>Payouts</Button>
            <Button variant="secondary" leftIcon={<Settings size={16} />} onClick={() => navigate('admin-finance-settings')}>Settings</Button>
            <Button leftIcon={<Plus size={16} />} onClick={() => navigate('admin-finance-invoice-create')}>Create Invoice</Button>
          </div>
        }
      />

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-4)' }}>
        <MetricCard title="Total Revenue" value="RM 2,450,000" icon={<TrendingUp />} trend="up" trendValue="+12% from last month" />
        <MetricCard title="Platform Commission" value="RM 45,000" icon={<CheckCircle />} />
        <MetricCard title="Outstanding Balance" value="RM 125,000" icon={<AlertCircle />} trend="down" trendValue="-5% from last month" />
        <MetricCard title="Pending Refunds" value="RM 12,000" icon={<Clock />} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)' }}>
        <div style={{ backgroundColor: 'var(--surface-base)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border-default)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button variant="secondary" size="sm" onClick={() => navigate('admin-finance-invoices')}>View All</Button>
          </div>
          <DataTable 
            data={invoices}
            ...
            isLoading={loading}
            onRowClick={(row) => navigate('admin-finance-invoice-details', { id: row.id })}
          />
        </div>
        ...
        <div style={{ backgroundColor: 'var(--surface-base)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border-default)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button variant="secondary" size="sm" onClick={() => navigate('billing-list')}>View All</Button>
          </div>
          <DataTable 
            data={payments}
