...
export const AdminPaymentList: React.FC<{ navigate: (route: string, data?: any) => void, showToast?: (title: string, desc?: string, variant?: 'success'|'error'|'warning'|'info') => void }> = ({ navigate, showToast }) => {
    ...
    { header: 'Date', accessor: 'date' as const, sortable: true },
    {
      ...
      accessor: (row: typeof paymentList[0]) => (
        <DropdownMenu
          ...
          iconOnly
          items={[
            { id: 'view_invoice', label: 'View Invoice', icon: <FileText size={16} />, onClick: () => navigate('billing-invoice-details', { id: row.invoiceId }) },
            { id: 'receipt', label: 'Download Receipt', icon: <Download size={16} />, onClick: () => { if (showToast) showToast('Downloading Receipt', `Receipt for ${row.id} is generating...`, 'info') } },
            ...(row.status === 'pending' ? [
              { id: 'verify', label: 'Verify Payment', icon: <CheckCircle size={16} />, onClick: () => { if(window.confirm('Verify this payment? This action is audit-sensitive.')) handleVerify(row.id) } },
              { id: 'reminder', label: 'Send Reminder', icon: <Send size={16} />, onClick: () => { if(showToast) showToast('Reminder Sent', `Payment reminder sent to ${row.agency}.`, 'success') } }
            ] : [])
          ]}
        />
      ),
