...
export const AdminRefundList: React.FC<{ navigate: (path: string, state?: any) => void }> = ({ navigate }) => {
  ...
  const columns = [
    { accessor: (row: any) => row.id, header: 'Refund ID', sortable: true },
    { accessor: (row: any) => row.invoice?.travelAgencyId || 'Direct Customer', header: 'Travel Agency', sortable: true },
    { accessor: (row: any) => row.invoiceId, header: 'Invoice Ref' },
    { 
    ...
    { 
      accessor: (row: any) => (
        <DropdownMenu
          ...
          iconOnly
          items={[
            { id: 'view_invoice', label: 'View Invoice', icon: <FileText size={16} />, onClick: () => navigate('admin-finance-invoice-details', { id: row.invoiceId }) },
