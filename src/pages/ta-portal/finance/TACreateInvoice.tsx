...
export const TACreateInvoice: React.FC<{ navigate: (route: string, data?: any) => void, showToast?: (title: string, desc?: string, variant?: 'success'|'error'|'warning'|'info') => void }> = ({ navigate, showToast }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', minHeight: 'calc(100vh - 120px)' }}>
      <PageHeader 
        ...
        breadcrumbs={[{ label: 'Finance', onClick: () => navigate('ta-finance') }, { label: 'Invoices', onClick: () => navigate('ta-finance-invoices') }, { label: 'Create Invoice' }]}
        actions={
                ...
                <div style={{ padding: 'var(--space-6)', backgroundColor: '#fff', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--space-6)' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div className="text-body-bold">Al-Hijrah Travel & Tours</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                  <div style={{ padding: 'var(--space-4)', backgroundColor: 'var(--surface-sunken)', borderRadius: 'var(--radius-card)', border: '1px solid var(--border-subtle)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                        {formData.customerName ? <CheckCircle size={16} className="text-success" /> : <AlertCircle size={16} className="text-danger" />}
                        <span className="text-body">Customer details</span>
                      </div>
                    </div>
                  </div>
      ...
      <div style={{ marginTop: 'auto', position: 'sticky', bottom: '-32px', margin: 'auto -32px -32px -32px', backgroundColor: 'var(--surface-base)', borderTop: '1px solid var(--border-default)', padding: 'var(--space-4) var(--space-8)', display: 'flex', justifyContent: 'space-between', zIndex: 10, boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
          <Button variant="ghost" onClick={() => navigate('ta-finance-invoices')}>Cancel</Button>
          <Button variant="secondary" leftIcon={<Save size={16} />} onClick={() => { if(showToast) showToast('Saved', 'Invoice saved as draft', 'info'); navigate('ta-finance-invoices'); }}>Save as Draft</Button>
          {currentStep < steps.length - 1 ? (
            <Button onClick={handleNext}>Next Step</Button>
          ) : (
            <Button onClick={handleIssue} leftIcon={<Send size={16} />} disabled={!formData.customerName || !formData.customerEmail || formData.items.length === 0}>Issue Invoice</Button>
          )}
        </div>
      </div>
    </div>
