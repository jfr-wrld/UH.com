...
export const BookingCreate: React.FC<{ navigate: (route: string, data?: any) => void, showToast?: (title: string, desc?: string, variant?: 'success'|'error'|'warning'|'info') => void }> = ({ navigate, showToast  }) => {
  ...
  const SectionHeader = ({ id, title, icon: Icon, isComplete = false }: any) => (
    <div 
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-4)', backgroundColor: 'var(--surface-sunken)', borderRadius: openSections[id] ? 'var(--radius-md) var(--radius-md) 0 0' : 'var(--radius-md)', cursor: 'pointer', borderBottom: openSections[id] ? '1px solid var(--border-default)' : 'none' }}
